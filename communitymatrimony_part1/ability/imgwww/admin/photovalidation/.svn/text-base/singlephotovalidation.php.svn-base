	...
	for($i=1;$i<=10;$i++){
		if (($row['Photo_Status'.$i] == 0 || $row['Photo_Status'.$i] == 2) && trim($row['Normal_Photo'.$i]) != '' && trim($row['Thumb_Small_Photo'.$i]) != '' && $row['Thumb_Big_Photo'.$i] != '') {
			if (file_exists($varPhotoCrop75.$row['Normal_Photo'.$i]) && file_exists($varPhotoCrop150.$row['Thumb_Small_Photo'.$i]) && file_exists($varPhotoCrop450.$row['Thumb_Big_Photo'.$i])){
				...
				$varContent	.= "<td width='12%' align='center' class='smalltxt' ><a class='mediumtxt clr1' target=_blank href=".$confValues['IMAGEURL']."/admin/photovalidation/admincropphoto.php?id=".$row['MatriId']."&num=".$i.">Recrop photo </a></td>";
				$varContent	.= "<td width='1%' align='center' class='smalltxt' ><TEXTAREA name='".$row['MatriId']."_desc_".$i."'  ROWS='2' COLS='75' class=inputtext style=width:175px;>".$row['Description'.$i]."</TEXTAREA></td>";
				...
				$varContent	.= '<input type="hidden" name="name'.$varCount.'" value ='.$row["Name"].'" ><input type="hidden" name="paidstatus_'.$varCount.'" value ='.$row["Paid_Status"].'" > </td></tr><tr><td colspan="8" height="10"></td></tr>';
