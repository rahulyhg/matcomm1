	...
	if($varTotalRecs > 0){
		...
		foreach($arrMsgDet as $varMsgId=>$arrSinMsgDet)
		{
			...
			switch($varMsgOptionVal){
				...
				case 'I':	
					$varMsgTxt	= substr($arrExpressInterestList[$varCurrMsgDet], 0, 67).' ...'; 
					$varMsgTxt	=	stripslashes(str_replace('\n', '<br>',$varMsgTxtShort));
					if($varCurrStatus == 0){$varClass	= 'bld';}
			...
			if($arrCurrBVDet['PU'] == 1){
				...
				$varProfDiv		= '<div id="msgdiv" class="smalltxt clr fleft"><font class="'.$varClass.'">'. $arrMsgArrList[$varMsgOptionVal].' to</font> <font class="clr1 bld">'.$arrCurrBVDet['ID'].'</font> '.$arrProfDet[0].' yrs, '.$arrProfDet[1].$varProfCasteInfo.$varProfCity.'</div>';
				...
				$varProfDiv		= '<div id="msgdiv" class="smalltxt clr fleft"><font class="bld">'.$arrMsgArrList[$varMsgOptionVal].' from</font> <font class="clr1 bld">'.$arrCurrBVDet['ID'].'</font> '.$arrProfDet[0].' yrs, '.$arrProfDet[1].$varProfCasteInfo.$varProfCity.'</div>';
			...
			$varBviewHTML  .= '
					<div class="normdiv"><div id="cont'.$i.'" onmouseover="this.className=\'hoverdiv\';" onmouseout="this.className=\'\';"><div class="fleft" style="width:20px;padding-top:15px;"><input type="checkbox" name="mids[]" value="'.$varMsgId.'"></div><div class="fleft" style="width:30px;padding-top:15px;"><img src="'.$confValues['IMGSURL'].'/'.$arrIconInfo[0].'.gif" alt="'.$arrIconInfo[1].'"></div><div id="mesgdiv" class="fleft padtb10" onclick="javascript:getViewProfile(\''.$arrCurrBVDet['ID'].'\', \''.$i.'\', '.$varMsgIndexVal.', '.$varMsgId.', \'S\')" ><div id="mesgdiv1" class="fleft">'.$varProfDiv.'<div id="infdiv" class="smalltxt clr2 fleft">'.$objMessage->getDaysTextInfo($varCurrDtSent).'</div><br clear="all"><div id="msgdiv1" class="normtxt clr">"'.$varMsgTxt.'"</div></div><div id="mesgdiv2" class="fleft"><div id="smphdiv" class="fleft"><img src="'.$varPhotoURL.'" width="25" height="25" border="0" alt=""/></div></div></div><br clear="all"/><div class="fleft"><img src="'.$confValues['IMGSURL'].'/trans.gif" height="1" width="90" /></div><div class="linesep rpanelinner"><img src="'.$confValues['IMGSURL'].'/trans.gif" height="1" width="1" /></div></div><div id="viewpro'.$i.'"></div></div>';
			$i++;
		}
