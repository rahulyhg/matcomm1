...
$varCurrentDate = date('Y-m-d H:i:s');
$varCommentwithAdmin = '--'.date('y-m-d H:i:s').'--'.$varUsername.'--';
...
if(($_POST['frmAddedProfileSubmit']=='yes') && ($varCheckPassword=='yes'))
{
	...
	for($i=0;$i<$varNoOfRecords;$i++)
	{
		...
		if( $action == "add" && $varPublishFlag == '')
		{
			...
			$varInsertId			= $objMaster->insert($varTable['MAILMANAGERINFO'],$argFields,$argFieldsValues);

			$argFields 				= array('Publish','Profile_Published_On');
...
$argFields	= array('User_Name','MatriId','Country','Name','Nick_Name','Age','Gender','Religion','Annual_Income','Income_Currency','Residing_Area','Residing_City','Contact_Address','Marital_Status','Date_Created','About_Myself','Education_Detail','Occupation_Detail','CommunityId','CasteId','CasteText','SubcasteId','SubcasteText','GothramId','GothramText','Mother_TongueId','Mother_TongueText','Contact_Phone','Contact_Mobile','About_MyPartner');


if($UserId!='') {
	if($NameType==1) { $argCondition			= "WHERE Publish=0 AND MatriId='".$UserId."'"; }
	if($NameType==2) { $argCondition			= "WHERE Publish=0 AND User_Name='".$UserId."'"; }
} else {
...
for($i=0;$i<$varTotalRow;$i++)
{
	...
	$varTotalTable .= '<tr><td><table border="0" cellpadding="0" cellspacing="0" class="formborderclr" width="542" align="center"><tr><td valign="top" class="smalltxt boldtxt" colspan="4" style="padding-left:10px;">MatriId : '.$varNewProfileDetails[$i]['MatriId'].'</td></tr>';
	...
	$varTotalTable .= '<tr class="memonlsbg4"><td class=smalltxt style="padding:5px 5px 5px 5px" colspan="3"><input name="matriId'.$i.'" value="'.$varNewProfileDetails[$i]['MatriId'].'" type="hidden"><input name="communityid'.$i.'" value="'.$varNewProfileDetails[$i]['CommunityId'].'" type="hidden"><input name="action'.$i.'" value="add" type="radio">&nbsp;&nbsp;<font class="text"><b>Add</b></font>&nbsp;&nbsp;<input name="action'.$i.'" value="ignore" type="radio"><font class="text"><b>Ignore</b></font>&nbsp;&nbsp;<input name="action'.$i.'" value="reject" type="radio"><font class="text"><b>Reject</b></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font class="text"></td><td class=formlink1 style="padding:5px 5px 5px 5px" align="right">'.$varDuplicateEmailLink.'<a href="'.$varModifyUrl.$varNewProfileDetails[$i]['MatriId'].'" class="smalltxt boldtxt" target="_blank">Modify Profile</a></tr>';
