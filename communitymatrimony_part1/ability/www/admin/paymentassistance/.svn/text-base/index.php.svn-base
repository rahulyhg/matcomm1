...
function showCommLang()
{
	...
	$showCommLang .= "<select id='commlang' name='commlang[]' size='3' multiple class='select' style='width:80px;' $disableCommLang>";
	foreach($paLanguage as $key => $value)
	{
		if(in_array($key,$_REQUEST[commlang]))
			$selected = " selected='selected'";
		$showCommLang .= "<option value='$key' $selected>$key</option>";
		$selected = "";
	}
	$showCommLang .= "</select>";
return $showCommLang;
...
function fromdate_todate_payment($formname)
{
	if($formname == "form2")
	{
		...
		$fromdate_todate_payment = '<td align="right" class="smalltxt">From Date:</td><td><input type="text" name="fromdate"  value="'.$fromDateValue.'" readonly style="width:120px;" class=\'inputtext\' onclick="displayDatePicker(\'fromdate\', \'\', \'ymd\', \'-\');"  HIDEFOCUS></td><td class="smalltxt">To Date:</td><td>';
...
//// After Suresh recommendations
	 $argCondition = " where (SupportUserName='' or SupportUserName='".$adminUserName."')  and Followupstatus=0 and LockTime < DATE_SUB(NOW(),INTERVAL 10 MINUTE) and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or (EntryType=0)) ".$qurydate." ".$applang." ".$commLangCond." ".$commReg;
	}
	if($category == "2")
	{
		if($fdate != "" && $tdate!="")
...
<table width="100%" align=center valign="top" border=0 cellpadding=4 cellspacing=1 class="normaltxt1" style="border:solid 1px #d1d1d1;">
	...
	<tr>
		<td align="right" width="20%" class="smalltxt">Select Domain:</td><td width="30%">
		<?php
			if($countSD <= 0 && $countCL >= 1)
			{
		...
		<td>
			&nbsp; <input type='checkbox' id='abledisable' name='abledisable' value='abledisable' onClick='abledisableDomainLanguage()'> &nbsp; <br>
		</td>
	<td class='smalltxt' align='left' valign='middle'>
	<?php
		echo showCommLang();
	?>
	</td>
	...
	<tr>
	<td align="right" width="20%" class="smalltxt">Lead Source:</td><td width="30%">
		<?php
