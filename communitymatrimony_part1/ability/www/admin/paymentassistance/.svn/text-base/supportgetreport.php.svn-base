...
foreach($paymentoption_followup_status as $key=>$value)
{
	...
	*/
	...
	$argCondition = " WHERE FollowupStatus=".$key." and FollowupDate >= '".$today." 00:00:00' and FollowupDate <='".$today." 23:59:59' and SupportUserName='".$adminUserName."'";

	$rowcountcnt = $objSlave -> numOfRecords($varTable['PAYMENTOPTIONS'], 'MatriId', $argCondition);
...
$argCondition = " where FollowupStatus!=0 and PaymentDate >='".$yesterday."' and  PaymentDate <='".$yesterdayevening." ' and SupportUserName='".$adminUserName."'";
$paycnt = $objSlave -> numOfRecords($varTable['PAYMENTOPTIONS'], 'MatriId', $argCondition);
if($objSlave -> clsErrorCode == "CNT_ERR")
{
	echo "Database error";
}
