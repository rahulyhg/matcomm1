...
if($_REQUEST[commlang] != "")
{
	...
	{
		$currArrCount = count($paCommLanguage[$clValue]);
		foreach($paCommLanguage[$clValue] as $langKey => $MatriIdss)
		{
			$commLangCond .= " CommunityId = $MatriIdss or ";
		}
...
$argCondition = " where ".$qurydate." and (SupportUserName='".$uname."' or SupportUserName = '' ) and FollowupStatus=0  and LockTime < DATE_SUB(NOW(),INTERVAL 10 MINUTE) and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) ". $applang."".$commLangCond.$commReg;
...
if($category == "2")
{
	...
	$argCondition = " where ".$qurydate." and SupportUserName ='".$uname."' and  FollowupStatus = ".$ts." ". $applang." ".$commLangCond.$commReg;
	$totalcount = $objPaysAssMaster -> numOfRecords($varTable['PAYMENTOPTIONS'],'MatriId',$argCondition);
	...
	$argCondition = " where ".$qurydate." and SupportUserName='".$uname."' and FollowupStatus='".$ts."' ". $applang." and ((EntryType=1 and PaymentDate='0000-00-00 00:00:00') or EntryType=0) $commLangCond order by DateUpdated asc limit ".$offset.",1";
...
if($category=="4")
{
	...
	$argCondition = "where PaymentDate='0000-00-00 00:00:00' and (FollowupStatus = 0) and LockTime < DATE_SUB(NOW(),INTERVAL 10 MINUTE) and (SupportUserName = '' or SupportUserName='".$uname."')";
...
$memberStatisticsQuery	= $objSlaveMatri -> select($varTable['MEMBERSTATISTICS'],$argFields,$argCondition,0);
...
$pageLink = "supportcheckstatus.php?Total=".$totalcount."&fdate=".$fdate."&tdate=".$tdate."&language=".mysql_real_escape_string($_REQUEST['language'])."&category=".$category."&".$freelock."&ts=".$ts."&leso=".$_REQUEST['leso']."&country=".$_REQUEST['country']."&commlang=".$_REQUEST['commlang']."&regt=".$new."&topFiftySwitch=".$_REQUEST['topFiftySwitch']."&topFiftySwitch1=".$_REQUEST['topFiftySwitch1'];
...
<tr><td colspan=2>
   ...
   <tr><td>&nbsp;</td><td colspan='2' align='left'>
	...
	<!-- IVR folow starts --->
	<tr><td class="smalltxt" colspan=5>
	...
	<tr><td align="left" class="smalltxt">Order-ID&nbsp;&nbsp;&nbsp;&nbsp;</td><td align="left"><input type="text" name="orderId"  ID="orderId"  ReadOnly> <? if($ccode != 98) { ?><INPUT TYPE="checkbox" NAME="CountryselectedNew" value='98' id="CountryselectedNew"> Generate INR Order Id <?php } ?><div id="ordehide" align="left"><a href="javascript:;"
	onClick="orderidAjax('<?=$recdgen['MatriId']?>',document.getElementById('PaymentCategory'),document.getElementById('Countryselected'),document.getElementById('PaymentCategorynew'),document.getElementById('CountryselectedNew'));">Generate Order-ID</a></div></td></tr>
	</table>
	</td></tr>
	<!-- IVR folow ends --->
