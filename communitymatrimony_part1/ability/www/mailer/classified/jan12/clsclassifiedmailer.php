...
/* To remove all the Junk,Dulicate and store only Valid mailid**/
  ...
  //To create the new Record to store EmailId
    ...
    function triggerMail($mailsettings,$emailId,$fileName,$from,$replaceText){
		...
		$this->smtpp->localhost=$mailsettings['localhost'];;       /* Your computer address */
		...
		echo 'ttt'.$replyto;
		$templ=$message;
		if($this->smtp->SendMessage($replyto, array($emailId), array("From: $emailfrom", "Reply-to: $replyto", "Sender: info@communitymatrimony.com", "MIME-Version: 1.0", "Content-type: text/html", "X-Priority: ".$priority, "X-Mailer: PHP mailer", "To: ".$emailId, "Subject: $subject", "Date: ".strftime("%a, %d %b %Y %H:%M:%S %Z")), stripslashes($templ))) {
   ...
   //To Select the particular Record
	function selectRecord($tableName,$lastId=0) {
		$argFields=array('EmailId','Id');
		$argCondition=' WHERE Unsubscribe=0 ORDER BY ID ASC ';
        ...
        foreach($email as $key=>$value) {
		   if($value!='') {
			   $this->totalMailSent++;
				...
				$this->triggerMail($mailsettings,$value,$templateFile,$from,$replaceText);
