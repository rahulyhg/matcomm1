...
if ($_POST['intRegister']=='yes') {
	...
	$varFieldsValues = array("'".$varWeightKgs."'","'".$varWeightUnit."'","'".$varBodyType."'","'".$varComplexion."'","'".$varBloodGroup."'","'".$varRaasi."'","'".$varHoroscopeMatch."'","'".$varChevvaiDosham."'","'".$varEatingHabits."'","'".$varSmokeHabits."'","'".$varDrinkHabits."'","'".$varCurrentDate."'","'".$varCurrentDate."'");
	...
	if($varSpokenLanguages != '' || $varInterests != '' || $varHobbies != '' || $varFavourites != '' || $varSports != '' || $varFood != '') {
		...
		$varFieldsValues	= array("'".$varHobbies."'","'".$varInterests."'","'".$varFavourites."'","'".$varSports."'","'".$varFood."'","'".$varSpokenLanguages."'","'".$varCurrentDate."'");
	...
	if($varNoOfBrothers!='' || $varBrothersMarried != '' || $varNoOfSisters != '' || $varSistersMarried != '') {
		...
		$varFieldsValues = array("'".$varNoOfBrothers."'","'".$varBrothersMarried."'","'".$varNoOfSisters."'","'".$varSistersMarried."'","'".$varCurrentDate."'");
	...
	if($sessPublish == 0 || $sessPublish == 4) {
		...
		$argFieldsValues	= array("'".$varEditedFatherOcc."'","'".$varEditedMotherOcc."'","'".$varEditedFamilyOri."'","'".$varEditedAbtFamily."'");
		...
		$argFieldsValues	= array("'".$varEducationDetail."'","'".$varOccupationDetail."'");
	...
	if($varWeightKgs!=0 && $varBodyType!=0 && $varComplexion!=0 && $varBloodGroup!=0 && $varSpokenLanguages!='' && $varEducationDetail!='' && $varOccupationDetail!='' && $varEatingHabits!=0 && $varSmokeHabits!=0 && $varDrinkHabits!=0 && $varInterests!='' && $varHobbies!='' && $varFavourites!='' && $varSports!='' && $varFood!='' && $varEditedFamilyOri!='' && $varEditedMotherOcc!='' && $varEditedFatherOcc!='' && $varNoOfBrothers!=0 && $varNoOfSisters!=0 && $varEditedAbtFamily!='' && $varFromAge!=0 && $varHeightFrom!=0 && $varPartnerLookingStatus!='' && $varPartnerCountry!='' && ($varResidingIndia!='' || $varResidingUSA!='') && $varPartnerFoodChoice!='' && $varEducation!='') {
		...
		if($varCompleteProfile == 1 && $varUseCaste==1 && $varPartnerCaste==''){
			$varCompleteProfile = 0;
		}
		if($varCompleteProfile == 1 && $varUseSubcaste==1 && $varPartnerSubcaste==''){
			$varCompleteProfile = 0;
		}
	} else {
		$varCompleteProfile = 0;
	...
	echo $varPartnerAgeFrom.'==0 || '.$varPartnerHeightFrom.'==0 || ('.$varUseMaritalStatus.'==1 && '.$varPartnerLookingStatus.'==\'\') || '.$varPartnerMotherTongue.'==\'\' || ('.$varUseReligion.'==1 && '.$varPartnerReligion.'==\'\') || ('.$varUseDenomination.'==1 && '.$varPartnerDenomination.'==\'\') || ('.$varUseCaste.'==1 && '.$varSizeCaste.'>1 && '.$varPartnerCasteId.'==\'\') || ('.$varUseSubcaste.'==1 && '.$varSizeSubcaste.'>1&& '.$varPartnerSubcasteId.'==\'\') || ('.$_FeatureDosham.'==1 && '.$varPartnerChevvaiDosham.'==0) || '.$varPartnerEatingHabits.'==\'\' || '.$varPartnerEducation.'==\'\' || '.$varPartnerCountry.'==\'\' || ('.$varPartnerResidentIndiaState.'==\'\' && '.$varPartnerResidentUSAState.'==\'\') || '.$varPartnerResidentStatus.'==\'\'';
}
?>