...
if($_POST['abilityinfosubmit'] == 'yes' && $_POST['act']=='abilitydesc') { 
	$arrProfileInfo		= explode('^|',$_COOKIE["profileInfo"]);
	$arrProfileInfo[19]	= $_REQUEST['physicalStatus'];
	$varProfileInfoCookie= implode('^|',$arrProfileInfo);
	setcookie("profileInfo","$varProfileInfoCookie", "0", "/",$confValues['DOMAINNAME']);
}//if
