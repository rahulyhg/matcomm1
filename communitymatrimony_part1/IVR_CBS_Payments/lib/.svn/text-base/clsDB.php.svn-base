...
include_once ($_CBS_IncludeFolder."/conf/ip.cil14");
...
Class DB
{
	...
	function ErrorLog($argError, $argQuery='') {
		...
		$funFileName = "/var/log/cbslog/".date('d-m-Y')."_".$_SERVER['SERVER_ADDR']."db_community_prod_error_log.txt";
	...
	function ExecuteQryResult($query,$argResultSetArr){
		  ...
		  if (!$funExecute = mysql_query($query,$this->clsDBLink))
			{
				$this->clsErrorCode		= "INSERT_ERR";
				$this->ErrorLog(mysql_error(), $query);
			}else{
					if($argResultSetArr == 1){
					$funResults = mysql_fetch_assoc($funExecute);
					return $funResults;
