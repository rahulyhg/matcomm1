...
// Added to turn OFF assert warnings in error log
...
class SphinxClient
{
	...
	// Added for filtering by one attribute
	function ResetFilterbyAttr($attribute)
	{
		if((trim($attribute)!='')&&(is_string($attribute)))
		{
			$arayKeys = array_keys($this->_filters);
			$maxValKey = count($arayKeys)-1;

			for($i=0;$i<=$arayKeys[$maxValKey];$i++)
			{
				if($this->_filters[$i]['attr'] == trim($attribute))
				{
					// Reset filter here
	...
	// New function for returning partnerids 
	function RunQueriesMatchSummary ()
	{
		if ( empty($this->_reqs) )
		{
			$this->_error = "no queries defined, issue AddQuery() first";
		...
		// parse and return response
		return $this->_ParseSearchResponseMatchSummary ( $response, $nreqs ); 
	}
	
	/// parse and return search query (or queries) response
	function _ParseSearchResponseMatchSummary ( $response, $nreqs )
	{
		$p = 0; // current position
		$max = strlen($response); // max position for checks, to protect against broken responses
			...
			while ( $count-->0 && $p<$max )
			{
				...
				foreach ( $attrs as $attr=>$type )
				{
					...
					if ( $type & SPH_ATTR_MULTI )
					{
						...
						{
							list(,$val) = unpack ( "N*", substr ( $response, $p, 4 ) ); $p += 4;
							if($attr=="contactedlist")
							{
								$returnmatriidresultcontactlist.=",".$val;
	
							} 
							elseif($attr=="viewedlist" && strstr($returnmatriidresultcontactlist,$val) == FALSE)
							{  
								$returnmatriidresultviewlist.=",".$val;		   
	      						}
						}
					} else
					{
		...
		$this->_MBPop ();
		if(substr($returnmatriidresultviewlist,0,1)==","){
			$matchsummarylistarray[0]=substr($returnmatriidresultviewlist,1);
		}else{
			$matchsummarylistarray[0]= $returnmatriidresultviewlist;
		}
	
		if(substr($returnmatriidresultcontactlist,0,1)==","){
			$matchsummarylistarray[1]=substr($returnmatriidresultcontactlist,1);
		}else{
			$matchsummarylistarray[1]=$returnmatriidresultcontactlist;
		}
	
		return $matchsummarylistarray;
		// Match summary function ends here
