...
class Phone extends DB {
	...
	//viewed phone number by me Count
	function getTotalCntPhNoViewedByMe($argMatriId,$argStartLimit,$argNoOfLimit) {
		global $varTable,$varDbInfo;
		$funViewedMyPhoneNoCnt = 0;
	...
	//viewed phone number by me
	function getPhNoViewedByMe($argMatriId,$argStartLimit,$argNoOfLimit) {
		global $varTable;
		
		$funCondition			= "WHERE Opposite_MatriId=".$this->doEscapeString($argMatriId,$this);
		$funViewedMyPhoneNoCnt	= $this->numOfRecords($varTable['PHONEVIEWLIST'], 'Opposite_MatriId', $funCondition);

		if($funViewedMyPhoneNoCnt > 0) {
			$funFields		= array('MatriId');
			$funCondition	= "WHERE Opposite_MatriId=".$this->doEscapeString($argMatriId,$this)." ORDER BY Date_Viewed DESC LIMIT ".$argStartLimit.", ".$argNoOfLimit;
			$varResultSet	= $this->select($varTable['PHONEVIEWLIST'],$funFields,$funCondition,0);

			while($row	= mysql_fetch_assoc($varResultSet)) {
				$this->arrViewedPhoneNoByMe[] = $row['MatriId'];
			}
		}
		return $funViewedMyPhoneNoCnt;
	}
