...
class MailManager extends DB
{
	...
	function getDomainDetails($argMatriId){
		...
		$arrDomainDetail['MAILERTPLPATH']= '/home/product/community/www/mailer/templates/'.$varMailerTplPath;
	...
	function sendEmail($argFrom,$argFromEmailAddress,$argTo,$argToEmailAddress,$argSubject,$argMessage,$argReplyToEmailAddress)
	{
		...
		if (mail($argToEmailAddress, $argSubject, $argMessage, $argheaders,"-f noreply@bounces.communitymatrimony.com")) { $funValue = 'yes'; }//if
	...
	function sendProfileMarriageConfirmMail($argMatriId,$argReceiverName,$argServerUrl,$argReason,$argGender)
	{
		global $varDomainPart2;
		$funPhoneDetails		= '';
		$funCurrentDate			= date('Y-m-d H:i:s');
		$arrGetProductInfo		= $this->getDomainDetails($argMatriId);
		$arrGetEmailInfo		= $this->getDomainEmailList($argMatriId);
		$funFrom				= $arrGetProductInfo['FROMADDRESS'];
		$funFromEmail			= $arrGetEmailInfo['INFOEMAIL'];
		$funToEmail				= $arrGetEmailInfo['MARRIAGECONFIRMMAIL'];
		$funUserEmail			= $this->getEmail($argMatriId);
		$funReplyToEmail		= $arrGetEmailInfo['NOREPLYMAIL'];
		$funProductName			= $arrGetProductInfo['PRODUCTNAME'];
		$funUserPhone			= $this->getMobileNo($argMatriId);
		if($funUserPhone != '') { $funPhoneDetails = "Phone No. : $funUserPhone"; }
		
		$funSubject				= "Marriage fixed through CBS";
		$funMessage				= "Dear Team,</br></br>Here are the details of the member who deleted his/her profile through $funProductName"."Matrimony.com and his/her marriage is fixed through $funProductName Matrimony</br></br>User MatriId  : $argMatriId</br>Reason for deletion : Found a partner on $funProductName Matrimony</br>Deleted Date : $funCurrentDate</br>Email-ID : $funUserEmail</br>$funPhoneDetails";
	...
	function sendNotifyEmail($argTo,$argMessage,$argSubject,$argMatriId){
		...
		$funheaders		.= "Return-Path: <noreply@bounces.communitymatrimony.com> \n";
		$argheaders		= $funheaders;
		if (mail($argTo, $varSubject, $argMessage, $argheaders,"-f noreply@bounces.communitymatrimony.com")){$funValue = 'yes';}
		return $funValue;	
	}
	...
	function sendEmailwithCC($argFromEmail,$argToEmail,$argSubject,$argMessage,$argReplyTo,$argCC,$argBcc) {
		...
		if (mail($argToEmail, $argSubject, $argMessage, $argheaders,"-f noreply@bounces.communitymatrimony.com")) { $funValue = 'yes'; }//if
		$retValue = $funValue;
		return $retValue;
	}
