...
Class NewMailerBasicView extends DB
{
	public $clsRequestPhotofImage = 'noimg50_f.gif';
	public $clsRequestPhotomImage = 'noimg50_m.gif';

	//SELECT BASIC VIEW DETAILS
	function selectDetails($argMatriIdArr,$argPurpose)
	{
		global $varTable,$arrHeightList,$arrMaritalList,$arrEducationDisplay,$arrCountryList,$arrResidingStateList,$arrUSAStateList,$residingCityStateMappingList;
		
		$arrNotSpecified	= array(7,9997,9998,9999,423,519,611,668);
		$varMatchMatriIdStr	= "'".implode("','",$argMatriIdArr)."'";
		$argCondition	= " WHERE MatriId IN (".$varMatchMatriIdStr.") AND Publish=1";
		...
		if($argPurpose=='match') {
			$varTableName	= 'cbsmatchwatch.'.$varTable['MEMBERINFO'];
		} else {
			$varTableName	= 'cbsmatchwatch.'.$varTable['PHOTOMEMBERINFO'];
		}
		$arrMatriIdsDet = $this->select($varTableName, $funFields, $argCondition, 1);
		...
		for($i=0; $i<$funCountRecs; $i++)
		{
			...
			if($this->funDenomArrayFeature>1) {
				if((!in_array($arrMatriIdsDet[$i]['Denomination'], $arrNotSpecified)) && $arrMatriIdsDet[$i]['Denomination']!=0 && $arrMatriIdsDet[$i]['Denomination'] != '') {
					$funDenomination= $this->arrMWDenomList[$arrMatriIdsDet[$i]['Denomination']];
				}
			}

			if($this->funCasteArrayFeature>1) {
				if((!in_array($arrMatriIdsDet[$i]['CasteId'], $arrNotSpecified)) && $arrMatriIdsDet[$i]['CasteId'] != 0 && $arrMatriIdsDet[$i]['CasteId'] != '') {
					$funCaste	= $this->arrMWCasteList[$arrMatriIdsDet[$i]['CasteId']];
				}
			}

			if($this->funSubcasteArrayFeature>1) {
				if((!in_array($arrMatriIdsDet[$i]['SubcasteId'], $arrNotSpecified)) && $arrMatriIdsDet[$i]['SubcasteId'] != '' && $arrMatriIdsDet[$i]['SubcasteId'] != 0 ) {
		...
		if($funPhotoIds != '')
		{
			...
			$varPhotoTableName= 'communitymatrimony.'.$varTable['MEMBERPHOTOINFO'];
			$resPhotoIdsDet = $this->select($varPhotoTableName, $funFields, $funCondition, 0);
			$this->checkConnection();
	...
	{
		global $arrResidingStateList,$arrUSAStateList, $arrCountryList, $arrEducationDisplay,$arrOccupationList,$varOnlineUser,$confValues,$varTable;
		...
		if($argPurpose=='match' || $argPurpose=='photo') {
			...
			$funInboxLink		= '<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=viewprofile~id='.$funMatriId.'~msgfl=1~msgty=R~msgid='.$argMsgId.'" style="text-decoration:none; color:'.$varLinkClr.';">Reply</a>&nbsp;&nbsp;&nbsp;<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=viewprofile~id='.$funMatriId.'~msgfl=1~msgty=R~msgid='.$argMsgId.'" style="text-decoration:none; color:'.$varLinkClr.';">Decline</a>';
		} else if($argPurpose=='inbox_ei') {
			$varBasicView		= $this->funMailerTplPath."/inboxbasic.tpl"; //do change (template file)
			$funInboxLink		= '<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=viewprofile~id='.$funMatriId.'~msgfl=2~msgty=R~msgid='.$argMsgId.'" style="text-decoration:none; color:'.$varLinkClr.';">Accept</a>&nbsp;&nbsp;&nbsp;<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=viewprofile~id='.$funMatriId.'~msgfl=2~msgty=R~msgid='.$argMsgId.'" style="text-decoration:none; color:'.$varLinkClr.';">Decline</a>';
		}
		...
		if ($varBasicDetails['PH'] == '') {
			...
			$funPhotoLink	= $this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId;
		} else if($varBasicDetails['PH'] == 'P') {
			//photo available with protect status
			$funPhotoPath='';
			if($varBasicDetails['G']==2) { $funPhotoPath	= $this->funMailerImgPath.'/img50_pro_f.gif'; } else { $funPhotoPath	= $this->funMailerImgPath.'/img50_pro_m.gif'; }
			$funPhotoLink	= $this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId;
		} else {
			//photo available
		...
		$varProfileViewLink		= $this->funServerUrl."/login/index.php?".$varTrackDetail."redirect=".$this->funServerUrl."/profiledetail/index.php?act=fullprofilenew~id=".$funMatriId;
		
		
		if($argLink==1) {
			//paid member
			$funXtaLink			= '<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId.'" style="text-decoration:none; color:'.$varLinkClr.';">Send Message</a>';
			$funTransWidth		= "250";
		} elseif($argLink==0) { 
			//free member
			$funXtaLink			= '<a href="'.$this->funServerUrl.'/login/index.php?'.$varTrackDetail.'redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId.'" style="text-decoration:none; color:'.$varLinkClr.';">Express Interest FREE</a>';
			$funTransWidth		= "210";
		}else { $funXtaLink			= ''; }
