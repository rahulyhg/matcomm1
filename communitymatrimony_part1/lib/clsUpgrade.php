...
function CBS_UpgradeProfile($argMatriId,$argOrderId,$argCategory,$argAmount,$objDB,$varTotalDays,$varMessage,$varSubject,$varCurrency,$objPayment,$varIPLocation,$argPrePaymentTable) {
		global $arrPhonePackage,$varTable,$arrPrdPackages,$varDomainName,$arrAstroPackage,$arrPrefixDomainList,$varReebokOfferCatIds,$varReebokOffer;
		...
		## CHECK OFFER
		include("/home/product/community/www/payment/offerinfo.php");
	...
	if($varHighlightStatus==0){
    ...
    }elseif(($varCategory == 48) || ($varCategory>=7 && $varCategory<=9) || ($argPrePaymentTable=='autorenewcharge')){
        ...
        $varHighlightStatus  = 0;
		if($argPrePaymentTable!='autorenewcharge'){
		...
		##FREE PROFILE HIGH LIGHTER FOR  AUTO RENEWAL UPGRADED PROFILE
		if($argPrePaymentTable=='autorenewcharge'){
			if($varCategory==1){
				$varAddonValidDays = 'DATE_ADD('.$varValiddays.',INTERVAL 15 DAY)';
			}elseif($varCategory==4){
				$varAddonValidDays = 'DATE_ADD('.$varValiddays.',INTERVAL 20 DAY)';
			}elseif($varCategory==7){
				$varAddonValidDays = 'DATE_ADD('.$varValiddays.',INTERVAL 25 DAY)';
			}
		}
