...
include_once($varRootBasePath.'/lib/clsMailerBasicView.php');
...
class MailerMatchWatch extends MailerBasicView {
	...
	function updateMWSentDtls($argMatriId,$argCommunityId,$argMatchCount,$argMWType,$argMatchSentDate) {
		...
		$argFieldsValues= array($this->doEscapeString($argMatriId,$this),$this->doEscapeString($argCommunityId,$this),$this->doEscapeString($argMatchCount,$this),$this->doEscapeString($argMWType,$this),$this->doEscapeString($argMatchSentDate,$this));
	...
	function getMatchWatchMatriIds($varDomainId,$argDomainPrefix,$arrMWMatriIds)
	{
		...
		$combinedFunTables	= 'memberinfo as mi, memberlogininfo as mli';
	...
	function sendEmail($argFrom,$argFromEmailAddress,$argToEmailAddress,$argSubject,$argMessage,$argReplyToEmailAddress)
	{
		...
		$funheaders				.= "Reply-To: <noreply@bounces.communitymatrimony.com>\n";
	...
	function ProfileIamLooking($varCommunityId,$varMatriId,$varGender,$varPartnerSet,$varPurpose,$varCasteOrDivision,$varSubcaste,$varPaidStatus,$varMatchSentDateOnly)
	{
		...
		$argCondition		= "WHERE CommunityId = ".$varCommunityId." AND MatriId=".$this->doEscapeString($varMatriId,$this);
	...
	function sendMatchWatchMail($argMatriId,$argName,$argToEmail,$argPaidStatus,$argPartnerSet,$argPaidDate,$argValidDays,$argRemainingDays,$varExpiryDate,$arrProfileMatchId,$argCasteOrDivision,$argPurpose,$argMatchSentDate)
	{
		...
		if($varProfileBasicView != '')
		{
			...
			if($argPartnerSet==1)
			{
				$partnerLink	= "<a href='".$this->funServerUrl."/login/index.php?redirect=".$this->funServerUrl."/profiledetail/index.php?act=partnerinfodesc' style='font:normal 11px arial; color:".$varLinkClr."; text-decoration:none;'>Edit preference</a>";
				$partnerContent	= 'Your Partner Preference set by you.';
			}//if
			else
			{
				$partnerLink	= "<a href='".$this->funServerUrl."/login/index.php?redirect=".$this->funServerUrl."/profiledetail/index.php?act=partnerinfodesc' style='font:normal 11px arial; color:".$varLinkClr."; text-decoration:none;'>Add preference</a>";
