...
class Privilege extends MailerMatchWatch {
	...
	{
		global $arrResidingStateList,$arrUSAStateList, $arrCountryList, $arrOccupationList,$varOnlineUser,$confValues,$varTable,$arrMatriIdPre,$arrFolderNames;
		...
		//$varProfileViewLink		= $this->funServerUrl."/login/index.php?redirect=".$this->funServerUrl."/profiledetail/index.php?act=fullprofilenew~id=".$funMatriId;

		$varProfileViewLink		= $funServerUrl."/login/index.php?redirect=".$this->funServerUrl."/profiledetail/index.php?act=fullprofilenew~id=".$funMatriId;
		
		if($argLink==1) {
			//paid member
			$funXtaLink			= '<a href="'.$this->funServerUrl.'/login/index.php?redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId.'" style="text-decoration:none; color:'.$varLinkClr.';">Send Message</a>';
		} elseif($argLink==0) { 
			//free member
			$funXtaLink			= '<a href="'.$this->funServerUrl.'/login/index.php?redirect='.$this->funServerUrl.'/profiledetail/index.php?act=fullprofilenew~id='.$funMatriId.'" style="text-decoration:none; color:'.$varLinkClr.';">Express Interest FREE</a>';
		}else { $funXtaLink			= ''; }
	...
	#------------------------------------------------------------------------------------------------------------
    //BASIC DISPLAY for Chat For VMM
	function chatBasicDetailsForVMM($varBasicDetails,$argLink='')
	{
		global $arrResidingStateList,$arrUSAStateList, $arrCountryList, $arrOccupationList,$varOnlineUser,$confValues,$varTable,$arrMatriIdPre,$arrFolderNames;

		$varBasicView			= "/home/product/community/vmmeet/www/mailers/templates/chatbasic.tpl"; //do change (template file)
		...
		if($varUserPhotoDetail == 'PP') {
			$varPhotoLink       = "photo_req(2,'".$this->funImgsServerPath."')";
			$varReplacePhUrl	= $this->funImgsServerPath.'/images/img85_pro.gif';
		} else if($varUserPhotoDetail != '') {
            $varPhotoLink       = "photo_req(3,'".$this->funImgsServerPath."')";  
			$arrUserPhotoDetail = explode('~',$varUserPhotoDetail);
			$varReplacePhUrl	= $varPhotoUrl.'/'.$funMatriId{3}.'/'.$funMatriId{4}.'/'.$arrUserPhotoDetail[0];
		} else {
			$varPhotoLink       = "photo_req(1,'".$this->funImgsServerPath."')"; 
			$varGenderImg		= ($varBasicDetails[0]['G']==1)?'img85_phnotadd_m.gif':'img85_phnotadd_f.gif';
			$varReplacePhUrl	= $this->funImgsServerPath.'/images/'.$varGenderImg;
		}
		//$varProfileViewLink		= $this->funServerUrl."/login/index.php?redirect=".$this->funServerUrl."/profiledetail/index.php?act=fullprofilenew~id=".$funMatriId;
		...
		$funBasicXerox = str_replace("<--NAME-->",ucwords($varBasicName),$funBasicXerox);
		$funBasicXerox = str_replace("<--USERNAME-->",$funMatriId,$funBasicXerox);
		$funBasicXerox = str_replace("<--AGE-->",$varBasicAge,$funBasicXerox);
		$funBasicXerox = str_replace("<--HEIGHT-->",$varBasicHtUnit,$funBasicXerox);
		$funBasicXerox = str_replace("<--RELIGION-->",$varBasicReligionSubcaste,$funBasicXerox);
		...
		$funBasicXerox = str_replace("<--PHOTOURL-->",$varReplacePhUrl,$funBasicXerox);
		$funBasicXerox = str_replace('<--PHOTO_LINK-->',$varPhotoLink,$funBasicXerox);
		
		return $funBasicXerox;
	}//mailBasicDetailsForVMM
    function getChatVMMRegularDetails($argPurpose,$varProfileBasicResultSet,$argPaidStatus)
	{
		for($i = 0; $i < sizeof($varProfileBasicResultSet); $i++)
		{
			$funDispBasic=$this->chatBasicDetailsForVMM($varProfileBasicResultSet[$i],$argPaidStatus);
			$funDisplayBasicVal .= $funDispBasic; 
		}//while
		return $funDisplayBasicVal;		
	}//getChatVMMRegularDetails
}
