...
Class DB
{
	...
	//CREATE MYSQL CONNECTION WITH ALL DETAILS
	function dbConnection($argHostIP, $argDBUserName,$argDBPassword,$argDBName) {

		$DBLink = mysql_connect($argHostIP, $argDBUserName, $argDBPassword);
	...
	function numOfRecords($argTblName, $argPrimary='MatriId', $argCondition)
	{
		...
		if (!$funExecute)
		{
			...
			$this->SqlQueryLog($argTblName, $funQuery);
	...
	function update($argTblName, $argFields, $argFieldsValue, $argCondition)
	{
		...
		if(trim($argCondition) != "" && count($argFields)==count($argFieldsValue)){
			...
			if (!mysql_query($funQuery,$this->clsDBLink)) {
				...
				$this->ErrorLog(mysql_error(), $funQuery);
			} else { $this->SqlQueryLog($argTblName, $funQuery); return mysql_affected_rows($this->clsDBLink); }//else
		}
	...
	// WRITE SQL QUERY (SELECT, UPDATE) LOG TO FILE //
	function SqlQueryLog($argTblName, $argQuery='') {
		$funError		= debug_backtrace();
		$funFileContent	 = "\n".date('h:i:s').'#~#'.$funError[1]["file"].':~:'.$funError[1]["function"].'#~#'.$argQuery.'#~#'.$argError;
		if ($funError[1]["function"]=='update') { $varLogUpdate = 'update_qry_'; }
		$funFileName = "/var/log/dberrorlog/cbssqllog/".date('d-m-Y')."_".$_SERVER['SERVER_ADDR'].$varLogUpdate."-".$argTblName."_cbsqrylog.txt";
	...
	//Direct Query Execute Function
	function ExecuteQryResult($query,$argResultSetArr){
          //print "<br>".$query; exit;
		  if (!$funExecute = mysql_query($query,$this->clsDBLink))
			{
				$this->clsErrorCode		= "INSERT_ERR";
				$this->ErrorLog(mysql_error(), $funQuery);
			}else{
