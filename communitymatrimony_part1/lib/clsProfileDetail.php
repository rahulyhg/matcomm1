...
class ProfileDetail extends MemcacheDB
{
	...
	function getApproxOppGenCnt ($varCount) {
		if ($varCount <= 250) {
			...
			return floor($varCount/1000)*1000;
	...
	function getProfileInfo ($argMatriId) {
		...
		$this->dbConnect('S',$varDbInfo['DATABASE']);
		if ($argMatriId !="") {
			$argFields	= array('MatriId','Name','Gender','Publish','Paid_Status','Complete_Now');
			$argCondition = "WHERE MatriId=".$this->doEscapeString($argMatriId,$this);
	...
	function profilePendingTools() { // Profile completeness algorithim
		...
		if ($varGetCookieInfo['PHONEVERIFIED'] != 1 && $varGetCookieInfo['PHONEVERIFIED'] != 3) {
			$varPendingTools .= '<div class="fleft normtxt" style="width:125px !important;width:145px;padding-left:15px;"><a class="clr1" href="http://www'.$confValues['DOMAINNAME'].'/profiledetail/index.php?act=primaryinfo">Add Phone Number</a> <br> For instant response</div>';
		}
		if ($varGetCookieInfo['PHOTOSTATUS'] != 1 && trim($varPhotoUrl) == '') {
			$varPendingTools .= '<div class="fleft normtxt" style="width:100px !important;width:115px;padding-left:15px;"><a class="clr1" href="'.$confValues['IMAGEURL'].'/photo/index.php?act=addphoto">Add Photo</a> <br> For more clicks</div>';
		}
		if ($varGetCookieInfo['HOROSCOPESTATUS'] != 1 && $varGetCookieInfo['HOROSCOPESTATUS'] != 3 && $objClsDomain->useHoroscope() == 1) {
			$varPendingTools .= '<div class="fleft normtxt" style="width:125px !important;width:145px;padding-left:15px;"><a class="clr1" href="'.$confValues['IMAGEURL'].'/horoscope/index.php?act=addhoroscope">Add Horoscope</a> <br> For suitable matches</div>';
		}
		if ($varPendingTools != '') {
			$varPendingToolsLink = $varPendingTools;
		}
	...
	// To get IP address for a specific matri id
	function getProfileIpAddress($varMatriId) {
       global $varDbInfo,$varTable;
	   $this->dbConnect('S',$varDbInfo['DATABASE']);
	   $argFields=array('Ipaddress');
	   $argCondition = "WHERE MatriId =".$this->doEscapeString($varMatriId,$this);
	   $funQueryResultSet =$this->select($varTable['MEMBERINFO'],$argFields,$argCondition,0);
	   $funQueryResultSet = mysql_fetch_assoc($funQueryResultSet);
	   return $funQueryResultSet['Ipaddress'];
    }
	
	//get total count for who viewed my profile
	function getTotalCntWhoViewedMyProfile($argMatriId) {
		...
		if ($argMatriId !="") {
			$funCondition = "WHERE Opposite_MatriId=".$this->doEscapeString($argMatriId,$this);
			$paymentPageResult = $this-> numOfRecords($varTable['MEMBERPROFILEVIEWEDINFO'],'distinct(MatriId)',$funCondition);
		}	
