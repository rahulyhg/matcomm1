	...
	<meta name="Distribution" content="<?=$confPageValues['DISTRIBUTION']?>">
		...
		<script type="text/javascript" >
			...
			function JsLoadManage(){		
				...
				url			="<?=$confValues['IMAGEURL']?>/admin1/photovalidation/managephotoaddcrop.php?ID=<?=$sessMatriId;?>&photo75=<?=$varThumpImage75;?>&photo150=<?=$varThumpImage150;?>&photo450=<?=$varOriImageName;?>&phnum="+phnum+"&action="+action+"&photodesc="+desc+"&usertype=<?=$confUserType;?>";
			...
			function JsPhotoCongrats() {
				if (http_request.readyState == 4) {
					if (http_request.status == 200) {
						//alert(' JSCROP'+http_request.responseText);
						window.location="<?=$confValues['IMAGEURL']?>/admin1/photovalidation/photocongrats.php?ID=<?=$sessMatriId;?>&rand=<?=rand(1,1000)#top;?>";
					}else 
						alert('There was a problem with the request.');
				}
			}
