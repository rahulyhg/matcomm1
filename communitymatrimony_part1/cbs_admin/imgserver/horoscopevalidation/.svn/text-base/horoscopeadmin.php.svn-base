...
if($varPassedId != '') {
	...
	$varCondition		= " WHERE  MatriId=".$objSlaveDB->doEscapeString($varPassedId,$objSlaveDB);
	$varTotRecords		= $objSlaveDB->numOfRecords($varTable['MEMBERLOGININFO'], $argPrimary='MatriId', $varCondition);
	
	if ($varTotRecords == 1) {
		$varQueryCondition	= " AND b.MatriId=".$objSlaveDB->doEscapeString($varPassedId,$objSlaveDB);
	} else {
		$varCondition		= " WHERE  User_Name=".$objSlaveDB->doEscapeString($varPassedId,$objSlaveDB);
		$varTotRecords		= $objSlaveDB->numOfRecords($varTable['MEMBERLOGININFO'], $argPrimary='MatriId', $varCondition);
		if ($varTotRecords == 1) {
			...
			$varQueryCondition	= " AND b.MatriId=".$objSlaveDB->doEscapeString($varSelectLoginInfo['MatriId'],$objSlaveDB);
		} else {
			//Profile not found
			$varNoProfile	= 'yes';
		}
...
if($varNoProfile == 'no') {
	...
	echo '<body onload="clearRadio();"><form id="frmhorovalidation" name="frmhorovalidation" method="post" onsubmit="return validate();" action="'.$confValues['IMAGEURL'].'/admin1/horoscopevalidation/adminhoroscopeapprove.php"><table width="920" border="0" cellpadding="0" cellspacing="0" align="center" class="formborder"><tr class="adminformheader">
		...
		while($row = mysql_fetch_assoc($varResult)){	
				...
				if (($row['HoroscopeStatus'] == 0 || $row['HoroscopeStatus'] == 2) && trim($row['HoroscopeURL']) != '') {
					...
					if (file_exists($varPendingHRPath.$row['HoroscopeURL'])){
						...
						$varContent	.= "<td align='center'><a class='smalltxt clr1' target='_blank' 		href=".$confValues['IMAGEURL']."/admin1/horoscopevalidation/adminshowhoroscope.php?MATRIID=".$row["MatriId"]."&num=".$i.">".$row['MatriId']."</a></td>";
