	...
	{*/
		...
		for($i=0;$i<$totrec;$i++)
		{
			...
			if(addslashes(strip_tags(trim($_REQUEST[$action]))) == "Ignore")
			{
				...
				$argSupportFields 		= array('userid','matriid','editprofile','reporttype','validateddate');
				$argSupportFieldsValues= array($objMaster->doEscapeString($adminUserName,$objMaster),$objMaster->doEscapeString($_REQUEST[$martiId],$objMaster),3,7,"'".$varDateTime."'");
				$objMaster->insert($varTable['SUPPORT_VALIDATION_REPORT'],$argSupportFields,$argSupportFieldsValues);
			...
			{
				$argSupportFields=array('userid','matriid','editprofile','reporttype','validateddate');
				$argSupportFieldsValues= array($objMaster->doEscapeString($adminUserName,$objMaster),$objMaster->doEscapeString($_REQUEST[$martiId],$objMaster),2,7,"'".$varDateTime."'");				$objMaster->insert($varTable['SUPPORT_VALIDATION_REPORT'],$argSupportFields,$argSupportFieldsValues);
