...
/* Includes */
$DOCROOTBASEPATH = "/home/product/community";
...
if($_POST['sendmail'] && $flag!=1) {
	...
	$fromemail = get_support_user_email($objSlaveDB);
...
function get_support_user_email($objSlaveDB='') {
	global $varTable, $adminUserName, $varDbInfo;
	...
	$varQryRes     = $objSlaveDB->select($varTable['ADMINLOGININFO'],$argFields,$argCondition,1);
...
function sendMail($fromId, $toId, $ccId, $subject, $message, $attachment_array) {
    ...
    $headers .= "From: $email_from\n";
	$headers .= "Cc: $email_cc\n";
    $headers .= "Sender: info@communitymatrimony.com\n";
    $headers .= "X-Mailer: PHP\n"; // mailer
    $headers .= "Return-Path: <noreply@bounces.communitymatrimony.com> \n";
    $headers .= "Reply-To: <$email_from> \n";
...
<form name='frmsendemail' method='post' enctype="multipart/form-data" onsubmit='return validatesendmail()'>
	<table>
		<tr><td class=rfnt>Subject</td><td><input type='text' name='subject' size=40 value='' maxlength=100></input></td></tr>
		<tr><td class=rfnt>AttachFile</td><td><input type="file" name="fileatt"  id="at1"></td><?if($err!=""){?><td  align=center style="font:bold 12px arial;color:#ff0000;"><?=$err?></td><?}?></tr>
		<tr><td class=rfnt>Message</td><td><textarea name='message' rows=15 cols=40></textarea></td></tr>
		<tr><td colspan=2 align=center><input type=submit name=sendmail value='Send Email'></td></tr>
	</table>
