...
include_once($varRootBasePathh.'/lib/clsPhoneSupport.php');
...
$varPrivelageEmail		= trim($varPrivilegeInfo[8]);
...
function viewPermit($adminUserName='',$objDBSlave) {
	...
	$varCondition		= " WHERE User_Name=".$objDBSlave->doEscapeString($adminUserName,$objDBSlave);
...
function getRequestInfoSent($objDBSlave='',$varMatriId='') {
	global $varTable;
	$varCondition		= " WHERE SenderId=".$objDBSlave->doEscapeString($varMatriId,$objDBSlave)." GROUP BY RequestFor";
...
function getRequestInfoReceived($objDBSlave='',$varMatriId='') {
	global $varTable;
	$varCondition		= " WHERE ReceiverId=".$objDBSlave->doEscapeString($varMatriId,$objDBSlave)." GROUP BY RequestFor";
	$varFields			= array('COUNT(RequestId) AS COUNT', 'RequestFor');
...
function getBookMarkInfo($objDBSlave='',$varMatriId='') {
	global $varTable;
	$varCondition		= " WHERE matriid=".$objDBSlave->doEscapeString($varMatriId,$objDBSlave);
...
function getViewedPhoneListByMember($objDBSlave='',$varMatriId='') {
	global $varTable;
	$varCondition		= " WHERE Opposite_MatriId=".$objDBSlave->doEscapeString($varMatriId,$objDBSlave);
	$varFields			= array('COUNT(matriid) AS COUNT', 'matriId');
	$varExecute	        = $objDBSlave->select($varDbInfo['DATABASE'].'.'.$varTable['PHONEVIEWLIST'],$varFields,$varCondition,0);
...
function getPaymentHistory($objDBSlave,$varMatriId) {
	...
	$varCondition		= 'WHERE MatriId='.$objDBSlave->doEscapeString($varMatriId,$objDBSlave).' AND Product_Id<100 ORDER BY Date_Paid DESC Limit 1';
...
<?
	...
	if($varRecordsNumber==0 && $varCommunityId>0) {
		if($varCommunityId == '2503') {
			$varUserNameCond	= "CommunityId=2503 AND (User_Name=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." OR User_Name='MUS-".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)."')";
		} else {
			$varUserNameCond	= "CommunityId=".$varCommunityId." AND User_Name=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail);
		}
	...
	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Status=1";
	$varNotRepliedMsgRecievedCount	= $objProfileDetail->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Status=2";
	$varRepliedMsgRecievedCount		= $objProfileDetail->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Status=3";
	$varDeclineMsgRecievedCount		= $objProfileDetail->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);


	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Status=0";
	 $varNewMsgRecievedCount			= $objProfileDetail->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=6 OR Status=7 OR Status=8 OR Status=9)";
	...
	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Status=5";
	$varMailSentDeleteCount			= $objProfileDetail->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=1 OR Status=2 OR Status=7 OR Status=8)";
	$varMailSentReadCount			= $objProfileDetail->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=0 OR Status=6)";
	$varMailSentNotReadCount		= $objProfileDetail->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=3 OR Status=9)";
	...
	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=6 OR Status=7)";
	...
	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=1 OR Status=6)";
	$varInterestSentAcceptList		= $objProfileDetail->numOfRecords($varTable['INTERESTSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND (Status=3 OR Status=7)";
	...
	if($varRecordsNumber > 0) {
		...
		$argMCCondition			= "WHERE MatriId=".$objDBSlave->doEscapeString($varMatriId,$objDBSlave);
		...
		if($varPaidStatus==1) {
			...
			$varrCondition		= "WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Product_Id IN (1,2,3,4,5,6,7,8,9,48,56) ORDER BY Date_Paid DESC LIMIT 1";
		...
		$argCondition				= "WHERE MatriId=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Matchwatch!=0";
		...
		$argCondition			= "WHERE Complaint_On=".$objProfileDetail->doEscapeString($varMatriId,$objProfileDetail)." AND Request_Closed=0 GROUP BY Complaint_Tag";
								...
								<table border="0" width="100%">
									...
									<tr>
										<td class="smalltxt" colspan="2">
										 <? if ($sessUserType=='3' || $sessUserType=='4') { ?><a onclick="window.open('sendemail.php?eid=<?=base64_encode($varEmail)?>', '','height=600, width=480');" ><b>Send Email</b></a><? } ?>
										</td>
									</tr>
									<tr>
										<td class="smalltxt" colspan="2">
		...
		<? if (($sessUserType =='2') ||(in_array($varPrivelageEmail,$arrPaymentUsers))) { ?>
		...
		<table border="0" cellpadding="0" cellspacing="0" class="formborder" WIDTH="532">
			...
			<tr bgcolor="#FFFFFF">
			   <td valign="top" class="smalltxt" style="padding-top:5px;padding-left:10px;padding-right:10px;padding-bottom:10px;line-height:15px;text-align:justify;" width="60%">
