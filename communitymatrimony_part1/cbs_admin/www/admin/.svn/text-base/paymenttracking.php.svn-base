...
if ($varFromDate !="" && $varToDate !="") { $varCondition = " AND DATE(A.Date_Paid) >=".$objDB->doEscapeString($varFromDate,$objDB)." AND DATE(A.Date_Paid)<=".$objDB->doEscapeString($varToDate,$objDB);  }
elseif ($varFromDate !="") { $varCondition	= " AND DATE(A.Date_Paid)=".$objDB->doEscapeString($varFromDate,$objDB);  }
					...
					<td width="100%" style="padding-left:3px;">
						<table border="0" cellpadding="0" cellspacing="0" align="center" width="99%" class="formborder">
							...
							<tr>
								...
								<td class="smalltxt clr3" align="center"><a href="index.php?act=view-profile1&actstatus=yes&matrimonyId=<?=$varResults['MatriId'];?>" class="smalltxt clr1" target="_blank"><?=$varResults['User_Name'];?></a></td>
