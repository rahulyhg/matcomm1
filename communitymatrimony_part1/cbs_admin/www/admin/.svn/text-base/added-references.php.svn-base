...
// Include the files //
include_once($varRootBasePath."/conf/reference.cil14");
...
if($_POST['frmAddedReferenceSubmit']=='yes')
{
	...
	for($i=0;$i<$varNoOfRecords;$i++)
	{
		...
		$objCommon->clsPrimaryValue	= array($objCommon->doEscapeString($matriId,$objCommon));
		...
		if( $action == "reject")
		{
			...
			$objCommon1->clsPrimaryValue	= array($objCommon1->doEscapeString($matriId,$objCommon1));
		...
		if( $action == "add")
		{
			...
			$objCommon->clsPrimaryValue	= array($objCommon->doEscapeString($matriId,$objCommon),2,0);
			...
			for($j=0;$j<$varReqSenderCnt;$j++)
			{
				...
				$objCommon->clsPrimaryValue	= array($objCommon->doEscapeString($varReqSender[$j]['SenderId'],$objCommon));
