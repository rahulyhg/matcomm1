...
if($_POST["edit"]=="true" && $_POST["frmAddPaymentSubmit"] !="yes") {
	...
	$varCondition	= " WHERE (OrderId=".$objSlaveDB->doEscapeString($varUsername,$objSlaveDB)." OR MatriId=".$objSlaveDB->doEscapeString($varUsername,$objSlaveDB)." OR User_Name=".$objSlaveDB->doEscapeString($varUsername,$objSlaveDB).") ORDER BY Date_Paid DESC LIMIT 1 ";
