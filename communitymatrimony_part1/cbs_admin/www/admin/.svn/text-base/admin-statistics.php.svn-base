...
if(($_POST['frmStatisticsSubmit'] == "yes"))
{
	...
	if($vartype==2) 
	{
		$varCondition		= " WHERE User_Name=".$objSlave->doEscapeString($varUserName,$objSlave);
	...
	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND Status=1";
	$varNotRepliedMsgRecievedCount	= $objSlave->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND Status=2";
	$varRepliedMsgRecievedCount		= $objSlave->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND Status=3";
	$varDeclineMsgRecievedCount		= $objSlave->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);


	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND Status=0";
	$varNewMsgRecievedCount			= $objSlave->numOfRecords($varTable['MAILRECEIVEDINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=6 OR Status=7 OR Status=8 OR Status=9)";
	...
	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND Status=5";
	$varMailSentDeleteCount			= $objSlave->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=1 OR Status=2 OR Status=7 OR Status=8)";
	$varMailSentReadCount			= $objSlave->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=0 OR Status=6)";
	$varMailSentNotReadCount		= $objSlave->numOfRecords($varTable['MAILSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=3 OR Status=9)";
	...
	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=6 OR Status=7)";
	...
	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=1 OR Status=6)";
	$varInterestSentAcceptList		= $objSlave->numOfRecords($varTable['INTERESTSENTINFO'],'MatriId',$varCondition);

	$varCondition					= " WHERE MatriId=".$objSlave->doEscapeString($varMatriId,$objSlave)." AND (Status=3 OR Status=7)";
