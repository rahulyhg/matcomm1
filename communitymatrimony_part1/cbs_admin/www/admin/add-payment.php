...
if ($varSubmit== "yes") {
	...
	$varCondition	= " WHERE User_Name=".$objMasterDB->doEscapeString($varLoginUserName,$objMasterDB)." AND Password='".md5($varLoginPassword)."' AND User_Type<>2";
	$varCheckLogin	= $objMasterDB->numOfRecords($varTable['ADMINLOGININFO'], 'User_Name', $varCondition);

	if ($varCheckLogin == 1) {

		//USERNAME / MATRIID VALIDATION
		$varCondition			= " WHERE ".$varPrimary."=".$objMasterDB->doEscapeString($varUsernameMatriID,$objMasterDB);
		...
		if ($varMatriID	!=	0 && $varOrderProcess == 'no') {
			...
			$varCondition	= " MatriId = ".$objMasterDB->doEscapeString($varPaymentMatriId,$objMasterDB);
			//SETING MEMCACHE KEY
			$varProfileMCKey= 'ProfileInfo_'.$varPaymentMatriId;

			$varFields		= array('Last_Payment','Valid_Days','Paid_Status','Phone_Verified','Publish','Number_Of_Payments','Expiry_Date','Date_Updated','Special_Priv');
			$varFieldsValue	= array("'".$varCurrentDate."'",$objMasterDB->doEscapeString($varTotalDays,$objMasterDB),'1','1','1','Number_Of_Payments+1','DATE_ADD(\''.$varCurrentDate.'\',INTERVAL '.$varTotalDays.' DAY)','NOW()',$varSpecialPrev);
...
#paymentType		=> Online, Chennai....
			...
			$argFieldsValue	= array($objMasterDB->doEscapeString($varOrderId,$objMasterDB),$objMasterDB->doEscapeString($varPaymentMatriId,$objMasterDB),$objMasterDB->doEscapeString($varPaymentUsername,$objMasterDB),$objMasterDB->doEscapeString($varCategory,$objMasterDB),$objMasterDB->doEscapeString($varOfferProductId,$objMasterDB),'\'\'',$objMasterDB->doEscapeString($varAmountPaid,$objMasterDB),$objMasterDB->doEscapeString($varDiscountPercentage,$objMasterDB),$objMasterDB->doEscapeString($varActualAmount,$objMasterDB),$objMasterDB->doEscapeString($varCurrency,$objMasterDB),'\'\'',$objMasterDB->doEscapeString($varChequeDDNumber,$objMasterDB),$objMasterDB->doEscapeString($varPaymentMode,$objMasterDB),$objMasterDB->doEscapeString($varChequeDDNumber,$objMasterDB),$objMasterDB->doEscapeString($varChequeDate,$objMasterDB),$objMasterDB->doEscapeString($varBankName,$objMasterDB),$objMasterDB->doEscapeString($varPaymentPoint,$objMasterDB),$objMasterDB->doEscapeString($varDirectDeposit,$objMasterDB),$objMasterDB->doEscapeString($varReceiptNumber,$objMasterDB),$objMasterDB->doEscapeString($varReceiptDate,$objMasterDB),$objMasterDB->doEscapeString($varComments,$objMasterDB),"'".$varCurrentDate."'");
			$objMasterDB->insert($varTable['PAYMENTHISTORYINFO'], $varFields, $argFieldsValue);

			//INSERT PHONE NUMBER 
			$varFields			= array('MatriId','TotalPhoneNos','NumbersLeft');
			$varTotalPhone="TotalPhoneNos+".$arrPhonePackage[$varCategory];
			$varNumbersLeft="NumbersLeft+".$arrPhonePackage[$varCategory];
			$varFieldsValues	= array($objMasterDB->doEscapeString($varPaymentMatriId,$objMasterDB),$objMasterDB->doEscapeString($varTotalPhone,$objMasterDB),$objMasterDB->doEscapeString($varNumbersLeft,$objMasterDB));
