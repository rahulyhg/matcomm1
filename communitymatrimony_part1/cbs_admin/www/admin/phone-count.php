...
if(($varSubmit == 'yes')||($varThroughViewProfile=="yes")){
	...
	if($_REQUEST['primary'] == 'MatriId'){		
		...
		$varCondition	= "WHERE User_Name=".$objDB->doEscapeString($varUserName,$objDB);
	}
	...
	if($_REQUEST['InsertType'] == 'Edit'){
		...
		$varFieldsVal	= array($objDB->doEscapeString($_REQUEST['TotalCnt'],$objDB),$objDB->doEscapeString($_REQUEST['LeftCnt'],$objDB));
		...
		$varFieldsVal	= array($objDB->doEscapeString($varMatriId,$objDB), $objDB->doEscapeString($_REQUEST['TotalCnt'],$objDB), $objDB->doEscapeString($_REQUEST['LeftCnt'],$objDB));
